(ns swipe.core
  ...
  (:require [goog.dom :as dom]
            ...
            [swipe.motion :refer [compose bind]]
            [swipe.keypad :refer [get-letters]]
            [swipe.render :refer [draw-square log clear-canvas draw-letters
                                  display-results clear-results 
                                  display-str display-processing]]
            [swipe.ajax :refer [web-get]]
...
(defn load-worker [message-chan]
  ...
  (let [loading-chan (chan)]
   (go 
    (>! message-chan "Start downloading training.txt")
    (let [text (<! (web-get "/training.txt")) ]
      (>! message-chan "Downloaded training.txt")
      ...
      (let [tree-chan (build-tree-chan text)]
        (loop [status (<! tree-chan)]
          (let [status-type (first status)
            ...
            (if (= :info status-type)
               (do 
                 (let [[left total] values]
                 (>! message-chan (str "Building tree " (.round js/Math (* 100 (/ (- total left) total))) "%")))
                 (recur (<! tree-chan))))
            (>! message-chan "Built-tree")
            (>! loading-chan (first values)))))))
    loading-chan))
...
(defn loading [message-chan]
  ...
  sent. Returns a channel immediately and puts a value put on it once loading
  is complete."
  (let [loading-chan (chan)]
    (go 
